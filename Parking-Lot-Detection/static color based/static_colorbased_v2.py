#açı konusunda da incelendi bu kodda, eğimli park yeri fotolarında da 
#aslında eğim hesabı yapan algoritma da başarılı ya da en azından
#doğru kabul edildi çünkü öncesinde yapılan örnek görüntülerdeki 
#testlerde mantıklı sonuçlar bulunmuştu. Sorunun park yeri contruct 
#edilirken kartonların birleşme yerlerinin güzel yapılmamasından olduğunu
#düşünüyoruz. park yeri muntazam şekilde yapılınca açı hesabının doğru 
#olması bekleniyor. color based ile ilgilenmektense yolo ile object 
#deetction kısmını denemeye geçtik. 20.12 17:37 itibariyle
...
target_image_path = 'renk75_50cm.jpg'  # Replace with your  target image path
target_image = cv.imread(target_image_path)
target_image = cv.resize(target_image, (640, 480))  # Resize to 640x480 pixels
...
if object_width_in_pixels is not None and object_height_in_pixels is not None:
    ...
    cv.putText(target_image, f"Distance: {round(Distance, 2)} CM,  Angle: {round(Angle, 5)} degrees", (50, 50), cv.FONT_HERSHEY_SIMPLEX,  1, (255, 255, 255), 2)
