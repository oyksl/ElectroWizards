...
def main(x_0, y_0, deg_0, x_end, y_end, deg_end):
  ...
  #  from plot_utils import plot_arrow
    ...
    start_x = x_0  # [m]
    start_y = y_0  # [m]
    start_yaw = np.deg2rad(deg_0)  # [rad]

    end_x = x_end  # [m]
    end_y = y_end  # [m]
    end_yaw = np.deg2rad(deg_end)  # [rad]
    ...
    while min(path) < -1.4 or max(path) > 1.4:
    ...
    if non_solved ==1:
        ...
        while min(path_to_origin) < -1.4 or max(path_to_origin) > 1.4:
            curvature = curvature + 1
            path_to_origin_x, path_to_origin_y, path_yaw, mode, lengths = plan_dubins_path(start_x,
                                                                                           start_y,
                                                                                           start_yaw,
            ...
            path_from_origin_x, path_from_origin_y, path_yaw, mode, lengths = plan_dubins_path(0,
                                                                                           0,
                                                                                           end_yaw,
                                                                                           end_x,
                                                                                           end_y,
            ...
            path_to_origin = np.r_[path_to_origin_x, path_to_origin_y]
            path_from_origin = np.r_[path_from_origin_x, path_from_origin_y]
    ...
    if show_animation:
        ...
        plt.plot(path_x, path_y, label="".join(mode)+' (One Layer)')
        plt.plot(path_to_origin_x, path_to_origin_y, label='Double Layer to origin')
        plt.plot(path_from_origin_x, path_from_origin_y, label='Double Layer from origin')
        plot_arrow(start_x, start_y, start_yaw)
        plot_arrow(end_x, end_y, end_yaw)
        plt.legend()
        plt.grid(True)
